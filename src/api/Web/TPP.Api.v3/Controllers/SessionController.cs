...
namespace TPP.Api.Controllers.V2
{
    ...
    public class SessionController : TPPBaseController
    {
        ...
        [ProducesResponseType(typeof(SessionDto), 201)]
        [ProducesResponseType(typeof(SessionDto), 400)]
        public async Task<IActionResult> RemoveUserFromSession([FromBody, Required] SessionDto sessionDto)
        {
            try
            {
                if (sessionDto == null)
                ...
                foreach (var user in sessionDto.Users)
                {
                    await _service.RemoveUserFromSession(sessionDto.Id, user.Id);
                }
